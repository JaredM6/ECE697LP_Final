...
uint8_t acc_index = 0;
uint32_t ans;//new line
int count=0;//new line
int drink_time=0;//new line
...
//new code
    ...
    double average=0.2;//here we have to determine the longest time it takes during tilting
    /*for(int i=0;i<5;i++){//test loop
        cin>>t;
        sleep(1);
        cout<<"original state: "<<ans<<endl;*/
    if(z<=-0.65){
time(&end_time);
            if(difftime(end_time,start_time)>=1&&(count==1||count==0)){
                ans=m_ui_led_constant_set(100,0,0);//blue light goes up
            }
            count=0;
    }
    else if(z>-0.65&&z<0.3){
time(&end_time);
            if(difftime(end_time,start_time)>=average&&(count==3||count==0)){//enter sth that 	can distinguish swinging from drinking motion
            ans=m_ui_led_constant_set(100,0,0);//yellow light goes up
                if(count==3){
                    count=0;
                }
                else{
...
time(&end_time);
            if(difftime(end_time,start_time)>=1){
               drink_time+=1; 
               ans= m_ui_led_constant_set(100,0,0);;//red light goes up               
count+=1;
            }
    }
